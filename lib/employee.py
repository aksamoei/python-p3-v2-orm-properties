...
class Employee:
    ...
    def name(self, employee_name):
        '''set validated employee name'''
        if isinstance(employee_name, str) and len(employee_name) > 0:
    ...
    def job_title(self, job_title):
        '''set validated job title'''
        if isinstance(job_title, str) and len(job_title):
            self._job_title = job_title
        else:
            raise ValueError("The job title must be a non-empty string")
